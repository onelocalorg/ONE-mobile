...
export const AddBreakDownModal = (
  ...
  const {id, expense, revenue, remainingAmt, payout, profilt, userId, onSuccessFulData} =
    props || {};
  ...
  const AddUserList = (item: any) => {
    ...
    if (!found) {
      ...
      const newuserData = {...newItems, user_id: item.id};
  ...
  async function createPayoutAPI() {
    ...
    console.log('----------------userListArray1111-------------',userList[0]['pic'])
