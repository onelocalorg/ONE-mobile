...
import {calendarTime, copy, dummy, pinWhite} from '@assets/images';
...
export const EventDetailScreen = (props: EventDetailScreenProps) => {
  ...
  const onPurchaseTicket = async (
    ...
    const request = {
      amount: parseFloat(price?.replace('USD', '')) * 100,
      currency: 'usd',
  ...
  return (
    <View>
      <Loader
        ...
        showOverlay={purchaseTicketLoading}
      />
          ...
          <View>
            {tickets?.map(ele => (
              <View key={ele?.price.toString()} style={styles.rowOnly}>
                <Text style={styles.ticket}>{`${ele?.name} - ${formatPrice(
                  ele?.price,
