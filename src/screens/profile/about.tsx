...
import { Alert, Button, Dimensions, Image, ListRenderItem, Modal, Text, TextInput, TouchableHighlight, TouchableOpacity, View, useWindowDimensions, ScrollView, LogBox, KeyboardAvoidingView, Platform } from 'react-native';
import { createStyleSheet } from './style';
import { useStringsAndLabels } from '@app-hooks/use-strings-and-labels';
import { Pill } from '@components/pill';
import { eventWhite, gratitude, save, explorer, operator, serviceProvider, postImage, bucket, loginLogo, arrowLeft, pin, arrowDown, buttonArrow, addCard, buttonArrowBlue, downImg, closeCard, addGreen } from '@assets/images';
...
export const About = (props: AboutDataProps) => {
  ...
  const { about, skills, profileAnswers, idUser, onEditProfile, navigation, route } = props || {};
  ...
  const [postData, setDataEntries] = useState();
  ...
  async function updateAnswerAPI() {
    ...
    console.log('============https://app.onelocal.one/api/v1/users/profile/que-answers==============',);
    console.log('============Request==============', queAns);
    try {
      const response = await fetch('https://app.onelocal.one/api/v1/users/profile/que-answers', {
        method: 'PATCH',
        headers: new Headers({
  ...
  async function packageListAPI() {
    ...
    console.log(token)
    try {
      const response = await fetch('https://app.onelocal.one/api/v1/subscriptions/packages', {
        method: 'get',
        headers: new Headers({
          'Authorization': 'Bearer ' + token,
  ...
  async function userProfileUpdate() {
    ...
    console.log(idUser)
    try {
      const response = await fetch('https://app.onelocal.one/api/v1/users/' + idUser, {
        method: 'get',
        headers: new Headers({
          'Authorization': 'Bearer ' + token,
          'Content-Type': 'application/x-www-form-urlencoded' 
  ...
  async function packageDetailAPI(dataId: any) {
    ...
    console.log(dataId)
    try {
      const response = await fetch('https://app.onelocal.one/api/v1/subscriptions/packages/' + dataId, {
        method: 'get',
        headers: new Headers({
          'Authorization': 'Bearer ' + token,
          'Content-Type': 'application/x-www-form-urlencoded'
  ...
  async function onPurchaseAPI() {
    ...
    try {
      const response = await fetch('https://app.onelocal.one/api/v1/subscriptions/' + idPackage + '/purchase', {
        method: 'post',
        headers: new Headers({
          'Authorization': 'Bearer ' + token,
          'Content-Type': 'application/x-www-form-urlencoded'
  ...
  async function cancleSubscriptionAPI() {
    ...
    console.log(idPackage)
    try {
      const response = await fetch('https://app.onelocal.one/api/v1/subscriptions/' + idPackage + '/cancel', {
        method: 'post',
        headers: new Headers({
          'Authorization': 'Bearer ' + token,
          'Content-Type': 'application/x-www-form-urlencoded'
  ...
  const renderItem: ListRenderItem<string> = ({ item }) => (
    <Pill onPressPill={() => handleRemove(item)} pillStyle={styles.marginBottom} key={item} label={item} />
  );
  ...
  async function memberShipCheckoutAPI(dataId: any) {
    ...
    try {
      const response = await fetch('https://app.onelocal.one/api/v1/subscriptions/packages/' + dataId + '/checkout', {
        method: 'get',
        headers: new Headers({
          'Authorization': 'Bearer ' + token,
          'Content-Type': 'application/x-www-form-urlencoded'
      ...
      monthlyPrice(dataItems?.data?.customerBill?.billSummaryMonthly?.totalUnitsMonthly)
      yearlyPrices(dataItems?.data?.customerBill?.billSummaryYearly?.totalUnitsYearly)
  ...
  const ConfigListAPI = async () => {
    try {
      const response = await fetch('https://app.onelocal.one/api/v1/config/list', {
        method: 'get',
        headers: new Headers({
  ...
  async function CreateCardAPI(cardId: any) {
    ...
    try {
      const response = await fetch('https://app.onelocal.one/api/v1/subscriptions/cards/create', {
        method: 'post',
        headers: new Headers({
          'Authorization': 'Bearer ' + tokens,
          'Content-Type': 'application/x-www-form-urlencoded'
      ...
      <View style={styles.innerConatiner}>
        ...
        <GestureRecognizer
          ...
          <TouchableOpacity
            ...
            <Loader visible={false} showOverlay />
              {postData && Object.entries(postData).map(([key, val]: any) => {

                return <>
                  <View style={styles.packageDetailModal}>
                    <ScrollView showsVerticalScrollIndicator={false}
                      horizontal={false}>
                      <TouchableOpacity disabled activeOpacity={0} onPress={() => { openSettingsModal() }} >

                        <TouchableOpacity
                          style={getTextStyleDetail(val)}
                          // style={styles.container1}
                          activeOpacity={0.8}>
                          <ImageComponent source={{ uri: val.role_image }} style={[styles.icon]} />
                          <Text style={styles.label}>{val.title}</Text>
                        </TouchableOpacity>

                        <Text style={styles.playerDescription}>{val.description}</Text>
                        {/* <ImageComponent source={{ uri: val.background_image }} style={styles.postImageStyle}></ImageComponent> */}
                        {val.status == false ? <Subscription
                          label={strings.signUp}
                          pillStyle={styles.signUpStyle}
                          backgroundColor={val.color}
                          onPressPill={() => memberShipClick(val.id)}
                        // onPressPill={onOpenMemberShip}
                        /> : <View></View>}
                        <Text style={styles.playerText}>{val.defaultSignupText}</Text>
                        {val.status == true ? <TouchableOpacity onPress={cancleSubscriptionAPI}>
                          <Text style={styles.cancleSubStyle}>{strings.cancleSubscription}</Text>
              ...
              <Modal transparent={true} animationType="slide" visible={memberModal}>
              <Loader visible={isLoading} showOverlay />
                <View style={styles.packageModalMembership}>
                  <ScrollView showsVerticalScrollIndicator={false}
                    horizontal={false}>
                    <TouchableOpacity disabled activeOpacity={0} onPress={() => { memberShipHide() }}>
                      <Text style={styles.memberTitle}>{strings.membershipCheckout}</Text>
                      ...
                      <View style={styles.modalContainer}>
                        ...
                        {cardData?.brand ? <View>
                          ...
                          <View style={styles.cardList}>
                            ...
                            <Text style={styles.CardexpDate}>{cardData?.exp_month}{strings.slash}{cardData?.exp_year}</Text>
                          </View>
                        </View> : <View></View>}


                        <View style={styles.addViewcard}>
                          <Text onPress={onOpenModal} style={styles.addCard}>{strings.addCard}</Text>
                        </View>
                        <View style={styles.purchesButton}>
                          <TouchableOpacity
                            ...
                            <ImageComponent source={buttonArrow} style={styles.buttonArrow} />
                          </TouchableOpacity>
                        </View>
                      </View>
                ...
                <Modal transparent={true} animationType="slide" visible={addcard} style={styles.gesture}>
                <Loader visible={isLoading} showOverlay />
                  <KeyboardAvoidingView
                    ...
                    <View style={styles.addCardBorderContainer}>
                      ...
                      <View>
                        <TouchableOpacity onPress={() => { addCardModalHide() }}>
                          <Image style={styles.closeCardCont} source={closeCard}></Image>
                        </TouchableOpacity>
                        ...
                        <View>
                          ...
                          <TextInput placeholderTextColor="darkgray" value={cardnumber} maxLength={16} keyboardType='numeric' style={styles.addCardInput} placeholder='card number' onChangeText={value => {
                            console.log(value)
                          ...
                          <View style={styles.cardView}>
                            ...
                            <TextInput placeholderTextColor="darkgray" value={cardCvv} maxLength={3} keyboardType='numeric' style={styles.addCardCVCInput} placeholder='cvc' onChangeText={value => {
                              console.log(value)
                              cardCVVData(value)
                            }} />
        ...
        <View style={{ flex: 1 }}>
          {packageItem ? <FlatList keyExtractor={item => item.id}
            data={packageItem}
            ...
            renderItem={({ item, index }) => {
              return <View style={styles.containers}>
                <TouchableOpacity
                  ...
                  activeOpacity={0.8} onPress={() => { onPlayerClick(item.id) }}>
                  <ImageComponent source={{ uri: item.role_image }} style={[styles.icon1]} />
        ...
        {skillValue !== '' ? <TouchableOpacity style={{ zIndex: 1111111 }} onPress={() => { onAddSkill(skillValue) }}>
          <ImageComponent style={styles.skillAddImage} source={addGreen}></ImageComponent>
        </TouchableOpacity> : <View></View>}
        ...
        <View>
          <Modal transparent={false} animationType='slide' visible={openQues}>
            ...
            <View style={{ height: 'auto' }}>
              <View style={styles.HeaderContainer}>
                <TouchableOpacity onPress={() => { updateAnsBack() }}>
                  <ImageComponent source={downImg} style={styles.downIcon}></ImageComponent>
                </TouchableOpacity>
                <Text style={styles.MainContainer}>{strings.updateQuestion}</Text>
                <TouchableOpacity activeOpacity={0.5} onPress={() => { quesAnsModalHide() }}>
              ...
              <ScrollView showsVerticalScrollIndicator={false}
                ...
                <View style={styles.listContainer}>
                  {ansQueDataTwo ?
                    <FlatList data={ansQueDataTwo} renderItem={({ item, index }) =>
                      <View style={{ borderBottomWidth: 1, marginBottom: 12, borderBottomColor: 'gray' }}>
                        ...
                        <View style={{ height: 110 }}>
                          <TextInput editable={true} value={item.answer} multiline={true} style={styles.inputCont}
          ...
          <View>
            <TouchableOpacity onPress={() => { quesAnsModalOpen() }}>
              <Text style={styles.ProfileUpdateCont}>{strings.queAnsTitle}</Text>
            </TouchableOpacity>
            <View style={{ marginBottom: 30 }}>
              <FlatList data={ansQueData} renderItem={({ item }) =>
                <View>
                  {item.answer != '' ? <View>
                    <Text style={styles.questionsDisplayLbl}>{item.question}</Text>
                    <Text style={styles.answerDisplayCont}>{item.answer}</Text>
                  </View> : <View></View>}
                </View>
              }>
              </FlatList>
